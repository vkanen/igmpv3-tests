...
def mk_ip_hdr(s, d):
    ...
    ip_cs = 0x0000 #16 bits (not filled by kernel, raw sockets, must be calculated later)
    #ip_src #32 bits
    #ip_dst #32 bits
...
def mk_igmpv3_join_msg(src, group, src_list, filter_mode):
...
def mk_igmp_report(igmp_version, src, report_type, group, src_list, filter_mode):
    if igmp_version == 'v3':
    ...
    if igmp_version == 'v3':
        igmp = mk_igmp(src, group, src_list, filter_mode)
    else:
...
def add_vlan_tag(msg, vlan, pri, tpid):
    m=[]
    m = pack('!BBH', pri, vlan, 0x0800)
    m += msg
    return m
