...
parser = argparse.ArgumentParser(description='Send IGMPv3 leave msgs to 224.0.0.22 (all IGMPv3 routers)', 
                                 epilog='Example of use: leave_test.py -n 100')
parser.add_argument('-n', '--number', help='number of different mcgs to leave. The script will choose these automatically', 
                    type=int, default=1)
    ...
    def run(self):
        ...
        for i,j, k in product(range(int(a1),255),range(int(a2),255),range(int(a3),255)):
            if (not stop):
                ...
                igmp_r = mk_igmp_leave(args.source, group)
                if (args.dump == True):
                    dump_packet(igmp_r)
                slock.acquire()
...
igmp_leave_thread = igmp_t()
igmp_leave_thread.start()
